  ...
  const ImageCard = ({ image, flexClass }) => {
    ...
    return (
      <div 
        ...
        className={`relative group overflow-hidden rounded-[2rem] shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer bg-slate-200 aspect-[4/3] md:aspect-auto ${flexClass} flex items-center justify-center w-full`}
      >
        <img 
          src={image.src} 
          alt={image.alt} 
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
          <span className="inline-block px-3 py-1 bg-purple-600 text-white text-[10px] sm:text-xs font-black uppercase tracking-[0.1em] rounded-lg mb-2 shadow-md">
            {image.category}
          </span>
  ...
  return (
    <>
              ...
              if (chunk.length === 6) {
                ...
                return (
                  <div key={chunkIndex} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 w-full">
                    {chunk.map(img => (
                      <ImageCard key={img.id} image={img} flexClass="" />
                    ))}
                  </div>
                );
              }
      ...
      {mounted && selectedImage && createPortal(
        <div className="fixed inset-0 z-[999999] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 sm:p-8" style={{ position: 'fixed' }}>
          <button 
